...
@dim ═══════════════════════════════════════════════════════
All commands now use a leading slash. Type / followed by the command, e.g. /HELP.
Arrow Up/Down recalls command history. Tab completes commands.
...
@cyan /CONNECT BRE-XX CODE
  Connect to one remote BRE site server after NET is enabled.

@cyan /CLEAR
  Clear the CLI transcript; loaded data remains mounted.

@cyan /DIAGNOSTIC
  Open current base diagnostic dashboard.

@cyan /DISCONNECT
  Disconnect the active BRE site server.
...
@cyan /TOPOGRAPHY
  Open abstract wireframe facility overview (formerly /FACILITY STATUS).

@cyan /FX AUTO / /FX FULL / /FX LOW
  Adjust decorative visual effects intensity.

@cyan /HOME
  Display the corporate terminal home notice.
...
@cyan /NET ON / /NET OFF
  Toggle network-dependent terminal systems.

@cyan /PERFORMANCE ON / /PERFORMANCE OFF
  Toggle reduced decorative effects for smoother browsers.

@cyan /SEARCH
  Query mounted database titles and keywords.
  Example: /SEARCH perimeter-drone

@cyan /SITE STATUS
  Show the active BRE site connection state.
...
@cyan /STATUS FACILITY
  Open the facility overview through the status command.

@cyan /WELCOME
  Display the connected BRE site intranet index.
...
@red /FSEARCH <term>
  Fuzzy search titles, keywords, categories, and body text.

@red /LIST ALL
  Print the complete mounted database index.

@red /LOAD STATUS / /STATUS LOAD
  Load an external facility status profile.

@red /LOGOUT
  Terminate administrator session.

@red /STATUS CLEAR
  Restore default facility status data.

@dim Navigation: ↑↓ Menu | Enter Select | Esc Back
@dim ═══════════════════════════════════════════════════════
```
...
unknown_command_hint = Use /HELP to list available commands, or the menu on the left.
...
alarm = BIOSCAN ARRAY // VITALS
life = SHAREHOLDER VALUE // PROJECTION
