...
/**
 * Tokenize a line containing BBCode-style color markup into an ordered list of
 * segments. Lenient — unmatched open/close tags don't throw; they just affect
 * the boundary of the active color. Unknown color names (not in
 * {@link TERMINAL_COLOR_NAMES}) produce a segment with `className: null`.
 *
 * @param {string} text
 * @returns {Array<{text: string, className: string|null}>}
 *   `className` is `'t-amber'` etc. for known colors, or `null` for plain text.
 */
...
/**
 * Render `text` (which may contain color tags) into `element` instantly —
 * no typewriter animation. Plain segments become text nodes; colored segments
 * become `<span class="t-NAME">`. Used by every render path that bypasses the
 * typewriter (instant mode, transcript rebuild, mutable line, image lines).
 *
...
/**
 * Routing hub for setting the visible content of an output line. Inspects the
 * line text and dispatches to the right renderer:
 *   - Image-only markdown lines  → `renderImageLine` (emit <img>)
 *   - Lines with [color=...] tags → `renderColoredText` (text + colored spans)
 *   - Everything else            → plain `element.textContent`
 *
 * Used by every render path (instant, mutable, transcript rebuild, and the
 * typewriter's short-circuit for image/empty lines).
 *
...
/**
 * True iff `text` is an entire image-markdown line (no surrounding prose).
 * Uses the shared {@link IMG_LINE_RE} from utils.js. Tolerant of the leading
 * "| " gutter prefix that database.js may have added.
 *
 * @param {string} text
 * @returns {boolean}
 */
...
/**
 * Render a single image-markdown line as an `<img class="entry-image-embed">`
 * element. Returns `false` (without touching the element) if the text is not a
 * valid image line — callers should check with {@link lineIsImageMarkdown} first.
 *
 * @param {HTMLElement} element - target; existing children are cleared on success
 * @param {string} text - a full `![alt](data:image/...;base64,...)` line
 * @returns {boolean} true if an image was rendered, false otherwise
 */
...
/**
 * Rewrite `text` so that any `[color=X]...[/color]` region spanning a newline
 * gets closed/reopened on each physical line. This keeps the per-line print()
 * pipeline working when a colored region wraps across multiple lines in the
 * source markdown.
 *
 * Example: `[color=amber]foo\nbar[/color]` → `[color=amber]foo[/color]\n[color=amber]bar[/color]`
 *
 * @param {string} text
 * @returns {string}
 */
...
/**
 * Append a line to the output buffer and either render it instantly or queue
 * it for typewriter animation. The buffer drives both the visible output and
 * the page-up/page-down history.
 *
 * @param {string} text - line text (may contain inline color or image markdown)
 * @param {string} [className=''] - space-separated CSS classes for the wrapper div
 *   (typically a `.t-*` color, `entry-divider`, `cli-divider`, `entry-image-line`, etc.)
 * @param {Object} [options]
 * @param {boolean} [options.instant] - if true, render right now (no typewriter)
 * @param {string} [options.groupId] - opaque grouping id used by transcript pagination
 */
...
/**
 * Print a line (or multi-line block — newlines are split) to the terminal.
 * The single most-called function in this file; every command handler uses it.
 *
 * @param {string} text - one or more lines (split on `\n`)
 * @param {string} [className=''] - CSS classes applied to each emitted line
 *   (e.g. `'t-amber'` for warnings, `'t-red'` for errors, `'t-dim'` for hints)
 */
